...
def render_video(model_path, iteration, views, gaussians, pipeline, background, ape_code):
    # render_path_spiral
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, ape_code : int):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.feat_dim, dataset.n_offsets, dataset.fork, dataset.use_feat_bank, dataset.appearance_dim, 
        ...
        render_video(dataset.model_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, ape_code)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.ape)